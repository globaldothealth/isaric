...
<a name="immunosuppressed"></a>**has_immunosuppression** Bool. Is the subject immunosuppressed? This can be inferred as `True` if the patient is recorded as being on immunosuppressants pre-admission, which will also be recorded as a pre-admission medication at a later date, or could be directly recorded in the data as having an immunosuppression co-morbidity.
